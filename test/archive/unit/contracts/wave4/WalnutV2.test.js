...
describe("WalnutV2 - Deposit Flow", function () {
  ...
  describe("Borrow Function", function () {
    ...
    describe("Successful Borrows", function () {
      ...
      it("Should mint cUSDC tokens to user", async function () {
        const borrowAmount = ethers.parseUnits("500", 6);
        const encryptedAmount = await encrypt(borrowAmount);
        
        // Check initial cUSDC balance (should be 0)
        const initialBalance = await decrypt(await cUSDC.balanceOf(user1.address));
        expect(initialBalance).to.equal(0n);
        
        // Borrow
        await walnutV2.connect(user1).borrow(encryptedAmount);
        
        // Check cUSDC balance (should be 500 USD)
        const finalBalance = await decrypt(await cUSDC.balanceOf(user1.address));
      ...
      it("Should handle multiple borrows from same user", async function () {
        ...
        // Check cUSDC balance
        const balance = await decrypt(await cUSDC.balanceOf(user1.address));
    ...
    describe("Borrow Rejections (FHE.select - no revert)", function () {
      it("Should reject borrow exceeding LTV (no state change)", async function () {
        ...
        // Check cUSDC balance (should still be 0)
        const balance = await decrypt(await cUSDC.balanceOf(user1.address));
  ...
  describe("Repay Function", function () {
    ...
    it("Should not burn cUSDC or change debt for insufficient repayment", async function () {
    ...
    it("Should burn cUSDC and clear debt for sufficient repayment", async function () {
